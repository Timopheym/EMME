(function (exports) {
  ...
  exports.init = function (sio, sessionStore) {
    ...
    sio.on('connection', function (socket) {
      ...
      // Connect
      //
      socket.on('calculate', function (data, callback) {
          console.log(data,'----------------------------------------------');
          function puts(error, stdout, stderr) {
              var id = stdout.replace('\n','');
              console.log('Выводим результаты клиенту '+id);
              var modelClass = mongoose.model("Result");
              console.log(modelClass,'<---model class--->',id);
              if (modelClass != null){
                  modelClass.findById(id, function (err, r){
                      socket.emit('calculate',r);
                  });
              }
              sys.puts(error, stdout, stderr)
          }
          exec("java -classpath /var/www/emme/libs/mongo-2.7.3.jar:/var/www/emme/src/ emme "+data.id+" relese", puts);
          //teempla
          //exec("java -classpath /var/www/timopheym/EMMA/libs/mongo-2.7.3.jar:/var/www/timopheym/EMMA/src/ emme "+data.id, puts);
          console.log('----------------------------------------------');
      });
      // ----------------------------------------------------
      // Connect
      ...
      setUpCrudForModel('Artel', socket, hs);
    });
